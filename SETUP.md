...
1. **Connect this repo to CircleCI** — at [app.circleci.com](https://app.circleci.com/),
   set up the project and authorize the `tfparam` organization.
...
> Note: claiming the namespace `tfparam` is one-time and effectively permanent.

Dependency updates for this repo are handled by the self-hosted **Renovate**
runner in [`tfparam/tfparams`](https://github.com/tfparam/tfparams) (see that
